...
func update_health_bar(base_health):
	#hp_bar_tween.interpolate_property(node, parameter, start_value, end_value, duration, transition type, easing type
	hp_bar_tween.interpolate_property(hp_bar, "value", hp_bar.value, base_health, 0.1, Tween.TRANS_LINEAR, Tween.EASE_OUT)
	hp_bar_tween.start()
	if base_health >= 60:
